...
class BoundaryNodeExtractor:
    """
    Extracts boundary nodes and faces from 3D finite element meshes.
    
    This class provides functionality to identify and extract the boundary surface
    of 3D solid finite element meshes by finding faces that are not shared between
    adjacent elements. It supports various element types including tetrahedra,
    hexahedra, and wedge elements, handling both linear and higher-order elements.
    
    The class can extract:
    - Boundary node IDs that lie on the external surface
    - Boundary face connectivity for visualization or further analysis
    - Element-face mapping to identify which faces belong to which elements
    
    Supported element types:
    - Solid tetrahedra (linear and quadratic)
    - Solid hexahedra (linear, parabolic, cubic)
    - Solid wedges (linear, parabolic, cubic)
    - Thick shell elements (wedge and hex types)
    
    For 2D shell, plate, membrane, and plane elements, the class simply passes
    through all nodes since these are already boundary elements by nature.
    
    Example:
        >>> extractor = BoundaryNodeExtractor()
        >>> node = extractor.get_node_out()
        >>> element = extractor.get_elem_out()
    """
